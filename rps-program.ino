...
RPSChoice makeRPSChoice(){
  ...
  */
  RPSChoice choice;
    if (roundsPlayed == 0) { //round 1
      choice = RPS_Rock; 
      /*
      Statistically rock and scissors are the most common first choices at 35%,
      paper is played first 30% of the time. To maximize the chances of winning 
      rock will be played first, if rock or scissors is played, the game will be won or tied.
      */
    } else if (roundsPlayed == 1) { //round 2 will be completely randomized
      choice = randRPS(); 
    } else if (roundsPlayed == 2) { //round 3 will play the winning move assuming the opponent is following a pattern as follows
    //pattern 1, opponent is only playing 1 move every round
    //pattern 2, opponent is iterating through rock, paper and scissors. eg, after scissors and paper, rock will be expected so paper is played
      if (opponentChoices[0] == opponentChoices[1]) {
        choice = winningChoiceAgainst(opponentChoices[0]);
      } else if (opponentChoices[0] == RPS_Rock && opponentChoices[1] == RPS_Paper || opponentChoices[1] == RPS_Rock && opponentChoices[0] == RPS_Paper) {
        choice = winningChoiceAgainst(RPS_Scissors);
      } else if (opponentChoices[0] == RPS_Rock && opponentChoices[1] == RPS_Scissors || opponentChoices[1] == RPS_Rock && opponentChoices[0] == RPS_Scissors) {
        choice = winningChoiceAgainst(RPS_Paper);
      } else if (opponentChoices[0] == RPS_Paper && opponentChoices[1] == RPS_Scissors || opponentChoices[1] == RPS_Paper && opponentChoices[0] == RPS_Scissors) {
        choice = winningChoiceAgainst(RPS_Rock);
      } else {
        choice = randRPS();
      }
    } else if (roundsPlayed == 4) { //round 5 will be randomized unless the opponent has only played one move, in which case it will play the winning move
      for(int i = 0; i <= roundsPlayed; i++) {
        if(opponentChoices[i] != opponentChoices[0]) {
          choice = randRPS();
          return choice; 
        }
      }
      choice = winningChoiceAgainst(opponentChoices[0]);
    } else if (roundsPlayed == 9) { //failsafe check for round 10 if stuck in a loop
      choice = winningChoiceAgainst(opponentChoices[roundsPlayed-1]);
    } else { //round 4, 6, 7 and all else will follow the core strategy which relies on playing the rps choice that the opponent has played the most 
    int rock = 0, paper = 0, scissors = 0; //counters to track which play the opponent has used the most
      for(int i = 0; i <= roundsPlayed; i++) {
        if(opponentChoices[i] == RPS_Rock) {
          rock++; 
        }
        if(opponentChoices[i] == RPS_Paper) {
          paper++; 
        }
        if(opponentChoices[i] == RPS_Scissors) {
          scissors++; 
        }
      }
      if (rock > paper && rock > scissors) { //if any of the counters is highest, play that choice
        choice = RPS_Rock; 
      } else if (paper > rock && paper > scissors) {
        choice = RPS_Paper; 
      } else if (scissors > rock && scissors > paper) {
        choice = RPS_Scissors; 
      } else { //if none of the counters is highest, default to selecting the previous losing move strategy play
        choice = winningChoiceAgainst(opponentChoices[roundsPlayed-1]);
      }
    }
  return choice;
